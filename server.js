...
// ── Public: verify school code (used by Electron setup screen) ───────────────
// No auth required — only returns { ok, school } for registered codes.
app.get('/verify-school', async (req, res) => {
  try {
    const { query } = require('./src/db');
    const code = (req.query.code || '').toUpperCase().trim();
    if (!code) return res.status(400).json({ ok: false, error: 'code required' });
    const { rows } = await query(
      `SELECT id, name, city, state FROM schools WHERE id = $1`,
      [code]
    );
    if (rows.length === 0) return res.json({ ok: false });
    return res.json({ ok: true, school: rows[0] });
  } catch (err) {
    console.error('[verify-school]', err.message);
...
// ── POST /track/visit ─────────────────────────────────────────────────────────
// Called by index.html after Google login. Logs visitor identity + location.
// Uses ip-api.com (free, no key needed) for geolocation from server IP.
app.post('/track/visit', async (req, res) => {
  try {
    const { query } = require('./src/db');
    const { google_sub, name, email, device, browser, os, referrer } = req.body;
    if (!google_sub) return res.status(400).json({ ok: false });
    ...
    try {
      const geoRes = await fetch(`http://ip-api.com/json/${ip}?fields=city,regionName,country,countryCode`);
    ...
    await query(
      `INSERT INTO visitor_log
        (google_sub, name, email, ip, city, region, country, country_code, device, browser, os, referrer)
...
// ── GET /track/visitors ───────────────────────────────────────────────────────
// Admin-only: returns recent visitor log entries.
app.get('/track/visitors', async (req, res) => {
  const key = req.headers['x-admin-key'] || req.query.key;
  if (key !== process.env.ADMIN_API_KEY) return res.status(403).json({ error: 'Forbidden' });
  try {
    const { query } = require('./src/db');
    const { rows } = await query(
      `SELECT id, name, email, city, region, country, country_code,
              device, browser, os, referrer, visited_at
       FROM visitor_log
...
// ── Root redirect → teacher dashboard ────────────────────────────────────────
app.get('/', (req, res) => {
  res.redirect('/bhava-teacher-dashboard.html');
});
